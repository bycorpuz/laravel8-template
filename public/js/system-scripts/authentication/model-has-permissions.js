$(document).ready(function () {
    document.getElementById('breadcrumb').innerHTML = '<li>Developer</li><li>Add Permission to User</li>';
    ...
    var t = $('#table1').DataTable({
        ...
        order: [[ 1, 'desc' ]],
        ajax:{
            url: 'listmodelhaspermissions',
        },
        columns:[
            { "data": "action", "name": "action" },
            { "data": "created_at", "name": "model_has_permissions.created_at" },
        ...
        if($('#action_button').val() == 'Create'){
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'model-has-permissions/create',
                        method: "POST",
                        headers: {
    ...
    $(document).on('click', '.delete', function(){
        ...
        .then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: 'model-has-permissions/destroy/' + id,
                    method: "GET",
                    dataType: "json",
                    beforeSend: function(){
                        Swal.fire({
